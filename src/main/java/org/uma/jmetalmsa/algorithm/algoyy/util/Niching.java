...
public class Niching <S extends Solution<?>> {
	...
	public Niching(List<S> population, List<S> lastFront,
		...
		//this.mgPopulation = population.union(lastFront);
                ...
                mgPopulation.addAll(population);
                mgPopulation.addAll(lastFront);
		...
		for (int i = 0; i < mgPopulation.size(); i++) {
			...
			for (int j = 0; j < obj; j++) {
				...
				//sol.setNormalizedObjective(j, val);
                                sol.setAttribute("NormalizedObjective"+j, val);
			}
		}
	}
	...
	public double calNormlizedVDistance(Solution sol, double[] ref) {
		...
		for (int j = 0; j < obj; j++) {
			...
			//ip += sol.getNormalizedObjective(j) * ref[j];
                        ip += (double)sol.getAttribute("NormalizedObjective"+j) * ref[j];
		...
		for (int i = 0; i < sol.getNumberOfObjectives(); i++) {
			...
			//		* (sol.getNormalizedObjective(i) - d1 * (ref[i] / refLenSQ));
                        d2 += ((double)sol.getAttribute("NormalizedObjective"+i) - d1 * (ref[i] / refLenSQ))
					* ((double)sol.getAttribute("NormalizedObjective"+i) - d1 * (ref[i] / refLenSQ));
		}
		d2 = Math.sqrt(d2);
